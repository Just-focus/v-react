...
import { Props } from 'shared/src/ReactTypes';
    ...
    if(elementProps) {
      ...
      if(callbackNameList) {
        callbackNameList.forEach((callbackName, i) => {
          const callback = elementProps[callbackName] as EventCallback
          if(callback) {
            if(i === 0) {
              paths.bubble.unshift(callback)
            } else {
...
function triggerEventFlow(paths: EventCallback[], syntheticEvent: SyntheticEvent) {
  for(let i = 0; i < paths.length; i++) {
    ...
    // 使用调度器，根据优先级调度事件
    unstable_runWithPriority(eventTypeToSchedulerPriority(syntheticEvent.type), () => {
