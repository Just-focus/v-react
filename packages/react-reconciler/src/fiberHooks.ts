...
function areHookInputsEqual(nextDeps: EffectDeps, prevDeps: EffectDeps) {
  ...
  const prevDepsLength = prevDeps.length;
  const nextDepsLength = nextDeps.length;

  if (prevDepsLength !== nextDepsLength) {
    return false;
  }

  for (let i = 0; i < prevDepsLength && i < nextDepsLength; i++) {
